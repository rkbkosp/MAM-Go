...
*   **🤖 AI 人脸识别**: 客户端集成 AI 引擎，离线分析视频中的人脸，服务端实现自动聚类与人脸索引。
...
位于 `client` 目录。内嵌 AI 分析工具 (Python/Face Recognition)。
...
**AI 功能**:
客户端首次运行时，会自动释放 AI 分析工具到 `/tmp/mam_ai_tool_v1`。
转码的同时，AI 引擎会并行分析视频，提取人脸特征并上传至服务端。
服务端会自动将同一人脸聚类，并在审阅页面的 "People" 标签页展示。
```
